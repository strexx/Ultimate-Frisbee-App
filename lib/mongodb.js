...
function init(db) {
    ...
    var accountsCollection = db.collection('accounts'),
		...
		tournamentRoundsCollection = db.collection('rounds');
            ...
            for (url in responses) {
                ...
                if (response.body) {
                    ...
                    for (var key in responsesObj) {
						...
						var standingsArray = responsesObj[key].standings;

						for(var anotherKey in gamesArray) {
							objectsRounds.push(gamesArray[anotherKey]);
						}
						for(var anotherAnotherKey in standingsArray) {
							objectsRanking.push(standingsArray[anotherKey]);
						}
                    }
                }
            }

			//console.log(objectsRounds);

            //deleteTournamentRounds();
			//deleteTournamentStandings();
            //addTournamentRounds(objectsRounds);
			addTournamentRanking(objectsRanking);
        });

    }

	function deleteTournamentRounds() {
        tournamentRoundsCollection.drop(function(err, result) {
            if (err) {
                console.log(err);
            } else {
                console.log('Removed documents from the "matches" collection.', result);
    ...
    function addTournamentRounds(objects) {
        tournamentRoundsCollection.insert(objects, {
            keepGoing: true,
            continueOnError: true
        }, function(err, result) {
            if (err) {
	...
	function deleteTournamentRanking() {
        tournamentRoundsCollection.drop(function(err, result) {
            if (err) {
                console.log(err);
            } else {
	...
	function addTournamentRanking(objects) {
        tournamentRoundsCollection.insert(objects, {
            keepGoing: true,
            continueOnError: true
        }, function(err, result) {
